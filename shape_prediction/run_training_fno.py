...
def relative_l2_loss(pred, target):
    ...
    return (diff.norm(dim=1) / (tgt.norm(dim=1))).mean()
...
def main():
    ...
    for epoch in range(CONFIG["training"]["epochs"]):
        ...
        with torch.no_grad():
            for xb, yb, _ in val_loader:
                xb, yb = xb.to(device), yb.to(device)
                val_loss += relative_l2_loss(model(xb), yb).item() * xb.size(0)
    ...
    'Configuracao do conjunto de amostras':{
            'Tamanho do conjunto de amostras total': int(n_total),
            'Tamanho do conjunto de treino': int(n_train),
            'Tamanho do conjunto de validacao':int(n_total - n_train),
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir_figs, "loss_curves.pdf"))
    plt.close()
    print(f"Curvas salvas em {args.out_dir_figs}/loss_curves.pdf")
