...
'''
    ...
    with torch.no_grad():
        for xb, ab in val_loader:
            ...
            ab = ab.to(device)
            pred_a = model(xb)
            all_a_true.extend(ab.cpu().numpy().flatten())
            all_a_pred.extend(pred_a.cpu().numpy().flatten())

    all_a_true = np.array(all_a_true)
    all_a_pred = np.array(all_a_pred)

    # Scatter plot com todas as amostras
    plt.figure(figsize=(6, 5))
    plt.scatter(all_a_true, all_a_pred, alpha=0.3, s=5)
    lim = max(all_a_true.max(), all_a_pred.max())
    plt.plot([0, lim], [0, lim], 'r--', lw=2, label='Ideal')
    plt.xlabel('Amplitude Real')
    plt.ylabel('Amplitude Predita')
    plt.title(f'Real vs Predito ({len(all_a_true)} amostras)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, "amplitude_scatter.pdf"), dpi=150, bbox_inches='tight')
    plt.close()
    print(f" Scatter salvo em: {os.path.join(args.out_dir, 'amplitude_scatter.pdf')}")
'''
